...
app.addHook("preHandler", async (request, reply) => {
  ...
  const adminMutation = !methodIsSafe && /^\/v1\/(?:secrets|learning|agent-profiles|repositories|repository-providers|github-apps|model-oauth-sources|automation-git-sources|automation-responders|channel-routing-rules)/.test(request.url);
